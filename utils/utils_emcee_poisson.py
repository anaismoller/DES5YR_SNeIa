...
"""
2020 A. Moller
from github:anaismoller/DES_selection_function (5-year version)
...
def fit_MCMC(df, fit_param, path_plots):
    ...
    A_mcmc, alpha_mcmc, beta_mcmc = map(
    ...
    plt.plot(
        xx,
        sigmoid_func(xx, min_theta_mcmc[0], min_theta_mcmc[1], min_theta_mcmc[2])
        / theta_mcmc[0],
        color="yellow",
    )
    plt.plot(
        xx,
        sigmoid_func(xx, max_theta_mcmc[0], max_theta_mcmc[1], max_theta_mcmc[2])
        / theta_mcmc[0],
        color="yellow",
        label="1 sigma",
    )  # mcmc fit
    plt.plot(
        xx,
        sigmoid_func(xx, theta_mcmc[0], theta_mcmc[1], theta_mcmc[2]) / theta_mcmc[0],
        color="orange",
        label="Emcee sigmoid fit",
    )  # mcmc fit
    plt.errorbar(
        df["x"],
        df["ratio"] / theta_mcmc[0],
        yerr=df["err_ratio"],
        fmt="o",
...
def emcee_fitting(df, path_plots, min_var=15):
    ...
    # Initial params
    low_bounds = [0.1, 0.1, 2]
    high_bounds = [4, 10, 100]
    try:
        popt, pcov = curve_fit(sigmoid_func, df["x"].values, df["ratio"].values)
        lu.print_green("Functional initial guess", popt)
        low_bounds = [p - 3 * p / 10.0 for p in popt]
        high_bounds = [p + 3 * p / 10.0 for p in popt]
        ...
        plt.plot(
            xx,
            sigmoid_func(xx, popt[0], popt[1], popt[2]) / popt[0],
            color="orange",
            label="Functional sigmoid fit",
        )
        plt.xlabel(variable)
        plt.ylabel("ratio")
        plt.ylim(-0.1, 1.1)
        ...
        lu.print_blue("Emcee fitting sigmoid to data/simulation ratio (Poisson errors)")
        theta_mcmc, min_theta_mcmc, max_theta_mcmc = fit_MCMC(df, fit_param, path_plots)
        lu.print_green(f"emcee: {theta_mcmc} + {max_theta_mcmc} - {min_theta_mcmc}")
        lu.print_blue("Finished emcee")

        return theta_mcmc, min_theta_mcmc, max_theta_mcmc
    except Exception:
        lu.print_red("Ratio may be too flat for fitting a sigmoid")
